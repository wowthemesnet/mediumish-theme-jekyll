...
3. 각 token들을 이번에는 node들로 바꿔줍니다.
   - 여기에서 node(노드)란 Document Object Tree에서의 각 엔티티를 말합니다.
4. node들로 DOM tree를 만듭니다.
   - DOM은 각 노드들의 부모 자식 관계, 형제 관계 등을 파악할 수 있도록 해줍니다.
...
`<link rel="stylesheet" href="style.css">` 같은 코드를 통해 가져오는 `.css` 또한 raw byte 형태로 받게 되며, HTML의 raw byte로부터 DOM을 만들었듯 CSSOM(CSS Object Model)을 만드는 과정이 진행됩니다. DOM처럼 CSSOM도 바이트를 문자로 바꾸고, 토큰화하고, 노드로 바꾼 후 트리 구조로 완성됩니다.
